...
*
* Copyright (c) 2021-2022 by MAMEDUL ISLAM
*
...
(function (global, factory) {
	...
	var _ajax = function (ajx) {
		...
		var url = '';
		if (_isString(ajx.method)||_isNumber(ajx.method)) {
			url = ajx.url;
		}

		var method = 'GET';
		if ( _isString(ajx.method) && _isString(ajx.method) && ['GET', 'POST', 'PUT', 'DELETE', 'HEAD', 'OPTIONS', 'PATCH'].indexOf(ajx.method.toUpperCase()) > -1) {
		...
		var data = _isFunction(FormData) ? new FormData() : '';
		if (ajx.data && _isObject(ajx.data)) {
			for (var k in ajx.data) {
				if( _isFunction(FormData) ){
					data.append(k, ajx.data[k]);
				}else{
					try{
						data+= k=+encodeURIComponent(ajx.data[k])+'&';
		...
		xhr.withCredentials = _urlParse(url).hostname != window.location.hostname;
		...
		if (_isString(ajx.type)) {
			xhr.responseType = ajx.type;
		}

		xhr.open(method, url, true);

		if (_isString(ajx.method) && ajx.method.toUpperCase() == 'POST') {
			xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded; charset=UTF-8");
		}

		if ( _isString(ajx.username) || _isNumber(ajx.username) || _isString(ajx.password) || _isNumber(ajx.password) ) {
			var authType = "Basic";
			if ( _isString(ajx.authType) ){
				authType = ajx.authType;
			}
			var authLogin = "";
			if ( _isSet(ajx.username) && _isSet(ajx.password) ) {
				...
				authLogin = _encodeBase64(ajx.username);
			}else if ( _isUndefined(ajx.username) && _isSet(ajx.password) ) {
				authLogin = _encodeBase64(ajx.password);
			}
			xhr.setRequestHeader("Authorization", authType + ' ' + authLogin );
			authType = null;
			authLogin = null;
		}

		if (_isFlatten(ajx.cache)) {
			xhr.setRequestHeader("Cache-control", ajx.cache);
		}

		//default, no-cache, reload, force-cache, only-if-cached

		if (_isObject(ajx.headers) || _isArray(ajx.headers) ) {
			for (var k in ajx.headers) {
				try{
					xhr.setRequestHeader(k,ajx.headers[k]);
				}catch(err){}
			}
		}

		if (ajx.timeout && _isFlatten(ajx.timeout)) {
	...
	var functions = {
		...
		addAfter: function (html) {
			...
			for (var i = 0; i < this.length; i++) {
				try{
					if(_isObject(html)){
						if (_isSet(html.innerHTML) && _isSet(html.nodeName) && _isSet(html.nodeType) && html.nodeType==1) {
							if(_isSet(this[i].insertAdjacentElement)){
							...
							for(var n=0; n<html.length; n++){
								...
								} else if(_isFlatten(html[n]) && _isSet(this[i].insertAdjacentHTML) ){
							...
							for(var k in html){
								...
								} else if(_isFlatten(html[k]) && _isSet(this[i].insertAdjacentHTML) ){
									this[i].insertAdjacentHTML('afterend', html[k]);
								}
							}
						}
					} else if(_isFlatten(html) && _isSet(this[i].insertAdjacentHTML) ){
		...
		removeAttribute: function (attrName) {
			...
			for (var i = 0; i < this.length; i++) {
				if (this[i] && _isSet(this[i].removeAttribute)) {
					if(_isString(attrName)){
						...
						for(var k in attrName){
							if(_isSet(attrName.hasOwnProperty) && !attrName.hasOwnProperty(k)){continue;}
		...
		buildQuery: function () {
			...
			var elements = _isSet(args[0]) ? args[0] : ( _isSet(this[0]) && _isSet(this[0].elements) ? this[0].elements : this._ );

			if( _isObject(elements) ){

				if ( _isInt(elements.length) ) {
					...
					for(var k in elements ){
						if( _isSet(elements.hasOwnProperty) && elements.hasOwnProperty(k) ){
							try{
								data += k + '=' + encodeURIComponent(elements[k]) + '&';
							}catch(err){}
						}
					}
		...
		buildFormData: function () {
			...
			if( _isFunction(FormData) && _isSet(this[0]) && _isSet(this[0].elements) ){
		...
		xmlParse: function () {
			...
			if(_isSet( args[0] )){
				return _xmlParse( args[0] );
			}else{
				for( var i=0; i < this.length; i++ ){
					return _xmlParse( this[i] );
				}
				return _xmlParse( {} );
			}
		...
		htmlParse: function () {
			...
			var args = arguments||[];
			if(_isSet( args[0] )){
				return _htmlParse( args[0] );
			}else{
				for( var i=0; i < this.length; i++ ){
					return _htmlParse( this[i] );
				}
				return _htmlParse( {} );
			}
		...
		jsonParse: function () {
			...
			var args = arguments||[];
			if(_isSet( args[0] )){
				return _jsonParse( args[0] );
			}else{
				for( var i=0; i < this.length; i++ ){
					return _jsonParse( this[i] );
				}
				return _jsonParse( {} );
			}
	...
	var jDoms = function () {
		...
		jDoms.jDomsList.prototype.developer = developer;
		jDoms.jDomsList.prototype._ = selectors;
		jDoms.jDomsList.prototype.__ = context;
		jDoms.jDomsList.prototype.___ = root;

		for (var functionName in functions) {
			if ( _isSet(functions.hasOwnProperty) && !functions.hasOwnProperty(functionName)) {
				continue;
			}
		...
		for (var fncName in jDoms.fnc) {
			if (_isSet(jDoms.fnc.hasOwnProperty) && !jDoms.fnc.hasOwnProperty(fncName)) {
				continue;
			}
	...
	jDoms.isDefined = function(variable){ return _isDefined(variable); };
	...
	jDoms.isInteger = function(variable){ return _isInt(variable); };
	...
	jDoms.isUnique = function (arr) { return _isUnique(arr); };
	...
	jDoms.userAgent = function(){
		return _isSet(window.navigator) && _isSet(window.navigator.userAgent) ? window.navigator.userAgent : '' ;
	};

	jDoms.isCookie = function(){ 
		return _isSet(window.navigator) && _isSet(window.navigator.cookieEnabled) ? navigator.cookieEnabled : false;
	};

	jDoms.setCookie = function( cookieName, cookieValue, expireTime ){
		try{
			if(!(navigator ? navigator.cookieEnabled : false)) { return false; }
	...
	jDoms.geo = function(callback){
		...
		if ( _isSet(window.navigator) && _isSet(window.navigator.geolocation) ) {
	...
	jDoms.domsReady = jDoms.domReady;
