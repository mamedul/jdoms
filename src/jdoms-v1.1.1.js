...
(function (global, factory) {
	...
	var functions = {
		...
		getStyle: function (propertyName) {
			...
			for (var i = 0; i < this.length; i++) {
				if (_isString(propertyName)) {
					if (this[i].style && this[i].style[propertyName]) {
		...
		hasClass: function (className) {
			for (var i = 0; i < this.length; i++) {
				...
				} else if (this[i] && this[i].className) {
					// Fallback for older browsers
					currentClasses = (' ' + this[i].className + ' ').replace(/[\n\t]/g, ' ');
	...
	var jDoms = function () {
		...
		var qsAll = new jDoms.qsAll(selectors, context, root);

		jDoms.qsAll.prototype.constructor = jDoms;
		jDoms.qsAll.prototype.version = version;
		jDoms.qsAll.prototype.developer = developer;
		jDoms.qsAll.prototype._ = selectors;
		jDoms.qsAll.prototype.__ = context;
		jDoms.qsAll.prototype.___ = root;
		...
		for (var functionName in functions) {
			...
			jDoms.qsAll.prototype[functionName] = functions[functionName];
		}
		...
		for (var fncName in jDoms.fnc) {
			...
			jDoms.qsAll.prototype[fncName] = jDoms.fnc[fncName];
		}
	...
	jDoms.qsAll = function (selectors) {
		...
		try {
			...
			if (selectors instanceof Window) {
				domArr = [selectors];
				from = "window";
			} else if (selectors instanceof Document) {
				domArr = [selectors];
				from = "document";
